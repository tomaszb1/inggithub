...
RUN RELEASE=$(cat /etc/lsb-release  | grep DISTRIB_RELEASE  | cut -d= -f2)

#Install Google Cloud Components
RUN export CLOUD_SDK_REPO="cloud-sdk-$(lsb-release -c -s)"
...
RUN export GCSFUSE_REPO=gcsfuse-`lsb-release -c -s`
